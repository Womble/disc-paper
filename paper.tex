...
pytd@leeds.ac.uk}, Paola Caselli$^{1}$, Aaron Boley$^{2}$, Richard Durisen$^{3}$,\newline Tom Hartquist$^{1}$, John Ilee$^{1}$, Jonathon Rawlings$^{4}$ \\
...
\begin{figure*}
 ...
 \label{rhoT} 
 \caption{Left: A 3D plot of log number density (cm$^{-3}$) showing the spiral structure in the xy plane and scale height of the disc. Right: The 3D temperature structure of the disc; regions cooler than 40 degrees are not shown in 3D, demonstrating the narrow central region containing hot material.}
\end{figure*}
...
1. SHOW THE MOMENT 0 AND 1 ONLY MAPS OF C18O, OCS, H2CO AND HCO+ + SPECTRA TOWARD THE CENTER AND +/- 30 AU (FOR EACH SPECIES, YOU CAN DO A 3-PANEL FIGURE SHOWING THE TWO MOMENTS AND A PANEL WITH THE 3 SPECTRA AT DIFFERENT LOCATIONS)
+  THE CONTINUUM MAP 
3. SHOW THE SIMULATED ALMA OBSERVATIONS (SAME STYLE AS BEFORE) AND ALSO SHOW A SEPARATE FIGURE WITH THE PV DIAGRAMS OF ALL SPECIES. 
]

[NOTE: THE FOLLOWING PARAGRAPH IS NOT CLEAR AT ALL !!! PLEASE RE-WRITE MAKING SURE THAT THE SENTENCES MAKE SENSE AND THERE ARE NO TYPOS.  PLEASE MAKE THE FIGURES AS SUGGESTED AND THEN RE-WRITE THIS PART.]
%Simulated molecules were confined to speices where the chemical abundance in the disc and envelope are known, or have simulated abundances. In addition radative transfer simulations are limited to are limited to molecules where the collisional and Einstein co-efficients are known. Lines for these molecules were simulated with LIME for freqencies 50$\sim$500$\,$GHz, with focus on lines which could be observed in ALMA band 7 and molecules which are not typically seen in outflows, which havent been included in these simulations and could potentially contaminate disc observations. The results presented in this section represent those lines which show dectectable emission/absorption which can be used to trace either spiral structure or rotation.\newline
...
\begin{figure*}
 \includegraphics[width=168mm]{Figures/sim/imageOCS_28-27_30deg_composite_all.png}
 \label{OCS_all}
 \caption{OCS 28-27 {\bf Left:} Continuum subtracted integrated intensity map, {\bf Centre:} Continuum subtracted spectra from Y=0'', X=+0.1'' (bottom), X=-0.1'' (centre) and X=0'' (top) spaced by 10$\,$K for clarity, {\bf Right:} Intensity weighted velocity map}
\end{figure*}
...
\begin{figure*}
 \includegraphics[width=168mm]{Figures/sim/imageH2CO_4-0-4--3-0-3_composite_all.png}
 \label{h2co_all}
 \caption{H$_2$CO 4$_{0\,4}$ - 3$_{\,0\,3}$ {\bf Left:} Continuum subtracted integrated intensity map, {\bf Centre:} Continuum subtracted spectra from Y=0'', X=+0.1'' (bottom), X=-0.1'' (centre) and X=0'' (top) spaced by 50$\,$K for clarity, {\bf Right:} Intensity weighted velocity map}
\end{figure*}
...
\begin{figure*}
 \includegraphics[width=168mm]{Figures/sim/imageHCOp_1-0_30deg_composite_all.png}
 \label{h2co_all}
 \caption{HCO$^+$ 1-0 {\bf Left:} Continuum subtracted integrated intensity map, {\bf Centre:} Continuum subtracted spectra from Y=0'', X=+0.4'' (bottom), X=-0.4'' (centre) and X=0'' (top) spaced by 40$\,$K for clarity, {\bf Right:} Intensity weighted velocity map}
\end{figure*}
...
In order to make predictions on the observability of the synthetic brightness maps created by LIME, CASA (common astronomy software applications) was used to simulate observing these with the completed ALMA in it's most extended comfiguration for a total time of four hours.
...
\begin{figure}
 \includegraphics[width=54mm]{Figures/sim/casa_imageOCS_28-27_30deg_composite_ALMAwidth_big_dirty_PV_centre.png}
...
\begin{figure}
 \includegraphics[width=84mm]{Figures/sim/casa_imageOCS_28-27_30deg_composite_ALMAwidth_big_dirty_contSub.png}
...
\begin{figure}
 \includegraphics[width=54mm]{Figures/sim/casa_imageC17O_3-2_30deg_composite_ALMAwidth_dirty_PV_centre.png}
...
\begin{figure}
 \includegraphics[width=63mm]{Figures/sim/casa_imageHCOp_1-0_30deg_composite_ALMAwidth_dirty_PV_centre.png}

 \caption{HCO$^+$ 1-0 30 deg PV through centre simulated for ALMA most extended configuration}
\end{figure}

\begin{figure}
 \includegraphics[width=84mm]{Figures/sim/casa_imageHCOp_1-0_30deg_composite_ALMAwidth_dirty_contSub.png}
...
Outflows could contaminate measurements of rotation curves meaning that speices  which commonly trace outflows, such as CO and HCO$^+$ are not going to be good tracers of disc rotation. However speices such as OCS and C$^{18}$O which are not commonly seen in outflows (Yildiz et al 2012,)can be used to trace disc rotation.
...
In order to construct the grid, candidate points are randomly selected from the volume to be simulated. These candidates then have their density and molecular density compared against a reference point in order to decide if the point is to be used in the grid or not. [NOTE: THIS FIRST SENTENCE DOES NOT MAKE SENSE. PLEASE RE-WRITE IT AND MAKE SURE THAT IT IS CLEAR AND THERE ARE NO TYPOS.] Candidate grid points are selected at random in cylindrical co-ordinates, linearly spaced in z and $\phi$ and logarithmically spaced in r. For each point to be selected, a random number $\alpha$ is drawn from the semi-open set [0,$\,$1) as a threshold. After selection of random co-ordinates, the hydrogen density and molecular density at the candidate point (n and m, respectively) are compared against the densities of a reference point on the inner edge of the disc (n$_0$ and m$_0$). If $\alpha<\left( \frac{n}{n_0} \right)^{0.3}$ or $\alpha< \left( \frac{m}{m_0} \right)^{0.3}$ then the point is selected for use, otherwise another r, $\phi$, z co-ordinate is selected and this becomes the candidate point. The function comparing the candidate point to the reference point and the candidate point distribution were selected empirically to sample all the scales while ensuring that the majority of points are located in the inner disc where the density is higher. 20\% of these points are forced to be at radii greater than $\sqrt{R_{min}R_{max}}$ (where $R_{min}$ and $R_{max}$ are the inner and outer radius of the model) in order to stop too many of the selecected points clustering in the high density disc and leaving the envelope undersampled. In addiation to this method of selection, 5\% of the points are linearly distributed in x, y and z with no bias with regards to density or abundance. This provides a minimum level of sampling for the large low density regions in the outer parts of the simulated volume. See figure \ref{points} for an example of the points distribution in r, z. \newline
