...
\begin{figure}
 ...
 \caption{Model of the pre-stellar core L1544 used as the envelope of the young protoplanetary disc in the hybrid model. Showing temperature (green) and dust (red) temperature in kelvin, log number density (blue) in cm$^{-3}$ and inward velocity$\times$100 (cyan) in m$\,$s$^{-1}$. Adapted from Keto \& Caselli (2010) and Keto et al. (2013, in preparation).}
...
\begin{figure*}
 ...
 \caption{Column densities of OCS, H$_2$CO and HCO$^+$ used in the disc model. Figure adapted from I2011 [SHOULD WE ALSO ADD C17O COLUMN DENSITY AS WE WANT TO DISCUSS THAT LATER ?? ]}
 \label{Chemistry} 
\end{figure*}
...
\begin{figure}
 ...
 \label{grid}
 \caption{A plot of the points selected by the griding process and the paths down which photons can propagate overlaid on the density distribution (in m$^{-3}$). The points are more concentrated at small radii and in the densest regions.}
\end{figure}
...
(Note the integrated intensity and intensity weighted velocity maps (figures \ref{OCS_all} - \ref{hcop+_all} and \ref{mom0_maps}) were created by integrating between -12.5 to -0.5 km$\,$s$^{-1}$ and +0.5 to +12.5 km$\,$s$^{-1}$ to avoid being dominated by the contribution from the envelope, this can be seen in some PV diagrams  (figure \ref{PVs}) as the strong absorption feature at all positions around zero velocity, moment 1 maps are shown with a cut-off of 3$\sigma$ as described in section \ref{sec:alma_predictions})\newline
...
\begin{figure*}
 \includegraphics[width=168mm]{Figures/sim/imageOCS_26-25_30deg_all.eps}
...
OCS traces only the innermost 20$\,$AU of the disc and can be used to examine the central regions (figure \ref{OCS_all}). As OCS is not seen in outflows (e.g. Stanke 07, Van der Tak 03) it can be used to view rotation of the central part of the disc without risk of contamination from emission from shocked material. The OCS line, which has the highest excitation among the selected transitions, well traces the hottest and densest regions of the disc. As a result, the spiral structure is best seen in OCS (26 - 25), as shown in figure \ref{OCS_all}. The majority of the OCS line is visible in absorption against the bright continuum emission from the disc mid-plane. The exception to this is towards the centre of the disc where the hole in the hot, dense midplane means that there is little continuum to be absorbed.
...
The abundance of the H$_2$CO molecule is typical of the majority of the molecules simulated in I2011 in that it traces the spiral structure roughly the inner 40$\,$AU of the disc. The spiral structure of the disc can be seen clearly in the integrated intensity map and its rotation of detected out to larger radii than is possible with the OCS (figure \ref{h2co_all}). As with the OCS line, the majority of the H$_2$CO is seen in absorption against the disc mid-plane continuum with the central region seen in emission. Unlike the OCS however, the H$_2$CO extends out to large enough radii to be present in the voids between spiral arms. In these regions the H$_2$CO line can be seen in emission, this can be seen most clearly in the H$_2$CO channel map (figure \ref{h2co_chanmap}).

\begin{figure*}
  \includegraphics[width=168mm]{Figures/sim/imageHCOp_3-2_30deg_all.eps}  \caption{HCO$^+$ 3-2 {\bf Left:} Continuum subtracted integrated intensity map, {\bf Right:} Intensity weighted velocity map}
  \label{hco+_all}
\end{figure*}
...
HCO$^+$ in unique in the molecules simulated in I2011 in that it traces only the outer regions of the disc, and so can be used to look at the extended velocity and physical structure. In figure \ref{hco+_all} we can see that the HCO$^+$ line shows the outer edges of the spiral arms in absorption, but also shows emission from the diffuse gas further out in the disc. This allows the velocity structure of the disc measured out to larger radii.\newline
...
%With the exception of HCO$^+$, molecular lines are seen in absorption throughout the majority of the disc and in some cases a small amount of emission towards the centre. Molecular lines throughout the plane of the disc show up in absorption against the continuum emission of the shock heated, dense mid-plane. 
...
\begin{table}
 \centering
 \begin{minipage}{80mm}
   \caption{Sensitivities of ALMA given 6 hours integration time. As calculated by the ALMA on-line sensitivity calculator.}
   \label{sigmas}
